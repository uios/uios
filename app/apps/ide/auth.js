    ...
    change: (user) => {
        return new Promise((resolve, reject, url) => {
            if(user) {
                ...
                byId('commit-to-master').nextElementSibling.disabled = false;
                byId('commit-to-master').innerHTML = 'Commit to <b>master</b>';
                user.photoURL ? byId('avatar').innerHTML = byId('commit-avatar').innerHTML = '<div class="bkg cvr" style="background-image:url(' + user.photoURL + ')"></div>' : null; 
                if(document.body.dataset.path) { (['/', '/my/'].includes(window.location.pathname) ? '/home/' : window.location.pathname).router(); }
                else { window.location.pathname.router();}
            } else {
                document.body.removeAttribute('data-uid');
                byId('commit-to-master').nextElementSibling.disabled = true;
                byId('commit-to-master').innerHTML = 'Connect to <b>GitHub</b>';
                byId('commit-avatar').innerHTML = '';
